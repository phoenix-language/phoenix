...
|---|---|
| **M:N scheduler / schedulable I/O** — all code under VM scheduler management, cooperative parking | **Partial (2026-06-21):** in-tree scheduler types + single-thread park/resume harness (PR #104), schedulable I/O contract (PR #113), `IoWaitRegistry` / `AwaitIo` wakeup stub (PR #116), `AWAIT_IO` opcode contract in `vm-linear.md` (PR #127), M:N `WorkerPool` OS-thread harness (PR #136), I/O registry wired through worker pool (PR #139), AwaitIo opcode VM harness (PR #145), multi-context AwaitIo stress harness (PR #152). Remaining: std I/O park contract harness, Phoenix syntax, std I/O. M2's VM hardening; PHX-057's `ExecutionContext` boundary is the prerequisite refactor |
| **Full borrow checker** (`&T`/`&mut T` exclusivity, lifetimes) | **Partial (2026-06-21):** phase-0 exclusivity on trunk — overlapping `&mut T` (PR #106), shared `&T` + `&T`/`&mut T` conflict (PR #111, #115), if-arm join (PR #128), loop body / back-edge join (PR #131), golden diagnostics E2047/E2048 including if-arm and loop cases (PR #110, #119, #134), `phx explain` borrow codes (PR #135), call-argument overlapping `&mut T` (PR #146). Return-path join, lifetimes, and full CFG join remain post-MVP. M0/M1's fork-join ownership model — built CFG-shaped so the borrow checker can grow from it instead of replacing a linear tracker |
...
| **Bytecode source maps / debugger** (section 5 symbols) — **PHX-070** | **Partial (2026-06-21):** `PcSpanTable` codegen + CLI `format_vm_error` (PR #12–#13), link merge + integration span test, release profile strips section 5 (PR #101, #121, #123), nested/indirect callee PC map (PR #103), hostile section 5 verifier tests (PR #102), CLI release e2e (PR #124), release trap golden without spans (PR #130), multi-module linked callee span test (PR #137), function-name symbol stub (PR #153). Link/integration test flake fixes on trunk (PR #133, unique module-tree temp dirs, `std_result_match` build race). Remaining: CLI function-name display, full debugger. M8's IR span work (PHX-063) + M2's `(function_id, pc)` errors (PHX-056, done) |
