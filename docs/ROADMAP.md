...
| **Bytecode source maps / debugger** (section 5 symbols) — **PHX-070** | **Partial (2026-06-21):** `PcSpanTable` codegen + CLI `format_vm_error` (PR #12–#13), link merge + integration span test, release profile strips section 5 (PR #101, #121, #123), nested/indirect callee PC map (PR #103), hostile section 5 verifier tests (PR #102), CLI release e2e (PR #124), release trap golden without spans (PR #130), multi-module linked callee span test (PR #137). Link/integration test flake fixes on trunk (PR #133, unique module-tree temp dirs, `std_result_match` build race). Remaining: function-name symbol stub, full debugger. M8's IR span work (PHX-063) + M2's `(function_id, pc)` errors (PHX-056, done) |
